...
% Load parameters
load('/home/manzand/Documents/Python/neuralODE_examples/bounce_ball/odeffnn_bball_gen.mat')
% Contruct NeuralODE
...
% Setup
x0 = 30.0; % This is like the initial input to the ODeblock
% u = [0]; % Initial input chosen (no inputs as for dynamical plants)
unc = 0.1;

R0 = Star([x0-unc;0.0],[x0+unc;0.0]);
U = Star(1,1);
...
R6 = [];
for k=1:length(R5)
    rtemp = ode3.simReach('direct',R5(k),U,reachStep1,numSteps1);
    R6 = [R6 rtemp(end)];
end
...
xlabel('x_1');
ylabel('x_2');
saveas(f,'bballlinearP_gen.png');


save('reach_gen.mat','tb','Rout');