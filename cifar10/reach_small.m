...
%  - layer1: convolutional (1,16,3,1)
%  - layer2: batchnorm (16)
%  - layer3: relu (no weights)
%  - layer4: convolutioanl (1,16,3,1)
%  - layer5: batchnorm (16)
...
file_path = '../../../../Python/neuralODE_examples/odecnn_mnist_small.mat';
load(file_path); % Load neuralODe parameters 
% Contruct NeuralODE
% w1 = reshape(Wb{1},[3 3 1 16]);
w1 = permute(Wb{1},[4 3 2 1]);
b1 = reshape(Wb{2},[1,1,16]);
layer1 = Conv2DLayer(w1,b1,[0,0,0,0],[1,1],[1,1]);
w2 = reshape(Wb{3},[1 1 16]);
b2 = reshape(Wb{4},[1 1 16]);
m2 = reshape(Wb{15},[1 1 16]);
v2 = reshape(Wb{16},[1 1 16]);
layer2 = BatchNormalizationLayer('Offset',b2,'Scale',w2,'TrainedMean',m2,'TrainedVariance',v2,'NumChannels',16,'Epsilon',16);
layer3 = ReluLayer;
% w4 = reshape(Wb{5},[4 4 16 16]);
w4 = permute(Wb{5},[4 3 2 1]);
b4 = reshape(Wb{6},[1,1,16]);
layer4 = Conv2DLayer(w4,b4,[1,1,1,1],[2,2],[1,1]);
w5 = reshape(Wb{7},[1 1 16]);
b5 = reshape(Wb{8},[1 1 16]);
m5 = reshape(Wb{18},[1 1 16]);
v5 = reshape(Wb{19},[1 1 16]);
layer5 = BatchNormalizationLayer('Offset',b5,'Scale',w5,'TrainedMean',m5,'TrainedVariance',v5,'NumChannels',16,'Epsilon',16);
layer6 = ReluLayer;
layer7 = FlattenLayer;
