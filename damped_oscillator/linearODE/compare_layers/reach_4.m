...
% Load parameters
load('/home/manzand/Documents/Python/sonode/experiments/damped_oscillators_linear/node./1_4_20./model.mat');
% Contruct NeuralODE
...
w6 = Wb{11};
b6 = Wb{12}';
Aout = w6*w5*w4*w3*w2*w1;
Bout = b6 + w6*b5 + w6*w5*b4 + w6*w5*w4*b3 + w6*w5*w4*w3*w2*b1 + w6*w5*w4*w3*b2;
Cout = eye(states);
D = zeros(outputs,1);
...
xlabel('x_1');
ylabel('x_2');
saveas(f,'DampedOsc_4_traj1.png');
...
xlabel('x_1');
ylabel('x_2');
saveas(f,'DampedOsc_4_traj2.png');
