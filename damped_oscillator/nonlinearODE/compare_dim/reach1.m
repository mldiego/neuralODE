...
odeblock = NonLinearODE(3,1,@node1,reachStep,controlPeriod,C); % Nonlinear ODE plant 
...
xlabel('x_1');
ylabel('x_2');
saveas(f,'DampedOsc_1_traj1.png');

%% Reachability run #2
% Setup
odeblock = NonLinearODE(2,1,@node1,reachStep,controlPeriod,C); % Nonlinear ODE plant 
odelayer = ODEblockLayer(odeblock,controlPeriod,reachStep,true);
neuralode = NeuralODE({odelayer});
...
xlabel('x_1');
ylabel('x_2');
saveas(f,'DampedOsc_1_traj2.png');
...
xlabel('x_1');
ylabel('x_2');
saveas(f,'DampedOsc_1_traj3.png');
