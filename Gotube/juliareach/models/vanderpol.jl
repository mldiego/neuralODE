...
const ey₁ = [0.0, 1.0, 0.0, 0.0]
const ey₂ = [0.0, 0.0, 0.0, 1.0]

@taylorize function vdp_N2_mu1!(dx, x, p, t)
    local μ = 1.0
    x₁, y₁, x₂, y₂ = x
...
@taylorize function vdp_N2_mu2!(dx, x, p, t)
    local μ = 2.0
    x₁, y₁, x₂, y₂ = x
...
function vanderpolN2(; μ=1.0)
    if μ == 1.0
        X0 = (1.25 .. 1.55) × (2.35 .. 2.45) × (1.25 .. 1.55) × (2.35 .. 2.45)
        prob = @ivp(x' = vdp_N2_mu1!(x), dim: 4, x(0) ∈ X0)
    elseif μ == 2.0
        X0 = (1.55 .. 1.85) × (2.35 .. 2.45) × (1.55 .. 1.85) × (2.35 .. 2.45)
        X0 = split(X0, [7, 1, 7, 1])
